...
void BaseRPC::process()
{
    try
    {
        ...
        {
            COMM_LOG_INFO(rpc_info() + " first connect-------------\n");
        ...
        if (is_inner_cq_event_)
        {
            ...
            COMM_LOG_INFO(rpc_info() + " inner cq event");
            return;
        }
...
void BaseRPC::create_rpc_for_next_client()
{
    try
    {
        COMM_LOG_INFO(rpc_info() + " create_rpc_for_next_client;");
...
void RequestTradeDataRPC::start_monitor_request()
{
    try
    {
        COMM_LOG_INFO("RequestTradeDataRPC start_monitor_request");
...
void RequestTradeDataRPC::proceed()
{
    try
    {
       COMM_LOG_INFO(rpc_info() + " proceed");

       COMM_LOG_INFO("RequestTrade: " + request_info_.exchange() + "." + request_info_.symbol() 
                + ": " + std::to_string(request_info_.time()));


        TradeData trade_data;

        ReqTradeData req_trade = get_req_trade(request_info_);
...
void GetTradeStreamDataRPC::start_monitor_request()
{
    try
    {
        COMM_LOG_INFO(rpc_info() + " start_monitor_request");
