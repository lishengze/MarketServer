...
KlineData calc_mixed_kline(type_tick index, const vector<KlineData>& datas) 
{
    ...
    for( const auto& v : datas ) {
        ret.px_open = ret.px_open + v.px_open * v.volume;
        ret.px_high = ret.px_high + v.px_high * v.volume;
        ret.px_low = ret.px_low + v.px_low * v.volume;
        ret.px_close = ret.px_close + v.px_close * v.volume;
    }
    ret.px_open = ret.px_open / ret.volume;
    ret.px_low = ret.px_low / ret.volume;
    ret.px_high = ret.px_high / ret.volume;
    ret.px_close = ret.px_close / ret.volume;
    return ret;
}
